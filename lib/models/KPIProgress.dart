...
/// Progress is updated from Activity Management module (Module 2.2.6)
class KPIProgress {
  final String? id; // Changed to String for Firestore document ID
  final String kpiId; // Changed to String to reference KPI document ID
  ...
  // Convert KPIProgress object to Map for database storage
  ...
  // Create KPIProgress object from database Map
  factory KPIProgress.fromMap(Map<String, dynamic> map) {
    return KPIProgress(
      id: map['id']?.toString(),
      kpiId: map['kpi_id']?.toString() ?? '',
      preacherId: map['preacher_id']?.toString() ?? '',
      sessionsCompleted: map['sessions_completed'] as int? ?? 0,
      totalAttendanceAchieved: map['total_attendance_achieved'] as int? ?? 0,
      newConvertsAchieved: map['new_converts_achieved'] as int? ?? 0,
      baptismsAchieved: map['baptisms_achieved'] as int? ?? 0,
      communityProjectsAchieved: map['community_projects_achieved'] as int? ?? 0,
      charityEventsAchieved: map['charity_events_achieved'] as int? ?? 0,
      youthProgramAttendanceAchieved: map['youth_program_attendance_achieved'] as int? ?? 0,
  ...
  // Create a copy of KPIProgress with modified fields
