  ...
  Future<bool> approveSubmission(String activityId, String submissionId) async {
    ...
    try {
      ...
      if (activityQuery.docs.isNotEmpty) {
        ...
        batch.update(
          _db.collection('activities').doc(activityDoc.id),
          {
            'status': 'Approved',
        ...
        if (preacherId != null && preacherName != null) {
          ...
          if (kpiQuery.docs.isNotEmpty) {
            ...
            if (progressQuery.docs.isNotEmpty) {
              ...
              final currentSessions = (progressDoc.data()['sessions_completed'] ?? 0) as int;
              final currentAttendance = (progressDoc.data()['total_attendance_achieved'] ?? 0) as int;
              final expectedAttendance = activityData['expectedAttendance'] as int? ?? 0;
              
              batch.update(
                _db.collection('kpi_progress').doc(progressDoc.id),
                {
                  'sessions_completed': currentSessions + 1,
                  'total_attendance_achieved': currentAttendance + expectedAttendance,
                  'last_updated': FieldValue.serverTimestamp(),
                  'last_activity_id': activityId, // Audit: Track which activity
          ...
          batch.set(auditRef, {
            ...
            'preacherId': preacherId,
            'preacherName': preacherName,
            'paymentCreated': existingPayment.docs.isEmpty, // True if new payment created
            'paymentAlreadyExists': existingPayment.docs.isNotEmpty, // True if skipped
